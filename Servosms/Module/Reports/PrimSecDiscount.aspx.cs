...
namespace Servosms.Module.Reports
{
	...
	public partial class PrimSecDiscount : System.Web.UI.Page
	{
		...
		public void GetPerDiscount(string ProdName,string PackType,string TotalQty,string TotalRate,string CustID,string SchType)
		{
			...
			{
				SqlDataReader rdr = obj.GetRecordSet("select discount,discounttype from oilscheme where schprodid=0 and prodid=(select prod_id from products where prod_Name='"+ProdName+"' and pack_type='"+PackType+"') and cast(floor(cast(datefrom as float)) as datetime)<=Convert(datetime,'" + GenUtil.str2DDMMYYYY(Request.Form["txtDateFrom"].ToString()) + "',103) and cast(floor(cast(dateto as float)) as datetime)>=Convert(datetime,'" + GenUtil.str2DDMMYYYY(Request.Form["TextBox1"].ToString()) + "',103)");
				if(rdr.Read())
				{
